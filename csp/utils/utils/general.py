...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            ax.set_ylim(0, 1.01)
            fig.tight_layout()
    fig.savefig('PR_curve_3001.png', dpi=300)
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    squeezed_targets, cls_list, cls_nums = squeeze_targets(targets)
    tcls, tbox, indices, anchors = build_targets(p, squeezed_targets, model)  # targets
        ...
        if n:
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                ...
                if 1:
                    ...
                    t[np.repeat(range(n), cls_nums[tcls_npy]), list(itertools.chain(*cls_list[tcls_npy]))] = cp
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    scale = int(((x[2] - x[0])).cpu())
    color = [0, 255, 0]
    cv2.rectangle(img, c1, c2, color, thickness=1, lineType=cv2.LINE_AA)   
    if label:
        label = label[0]
        tf = max(tl - 1, 1)  # font thickness
        # t_size = cv2.getTextSize(label, 0, fontScale=scale/15, thickness=tf)[0]
        try: 
            int(label)  
            # c1 = c1[0], c2[1]
            # c2 = c1[0] + t_size[0], c2[1] + t_size[1]
            # label_y = c2[1]
            label_x = c1[0] + 4
        except:
            if label == 'l':
        ...
        # if label[0] == ''
        cv2.putText(img, label, (label_x, c1[1] - 2), 0, scale/12, [225, 255, 255], thickness=1, lineType=cv2.LINE_AA)
